...
int domain_count;
char extend_pass_domains[DOMAIN_MAX][DOMAIN_LEN];
#endif
  ...
  for (latest = NULL, res = daemon->resolv_files; res; res = res->next)
	...
	if (force || (statbuf.st_mtime != res->mtime) || statbuf.st_size != res->size)
	    ...
	    res->size = statbuf.st_size;
			/* For we have only one `/tmp/resolv.conf` file, so we check the file whether modified or not by comparing the `mtime`( != ), NOT after(>) or before(<); Because the system time is adjusted when NTP updating. */
#if 1
			latest = res;
#else
...
#ifdef ENABLE_INSIGHT_LOG
int load_extend_pass_domain()
{
	int i;
	FILE *fd;
	char line[DOMAIN_LEN];

	for(i = 0; i < DOMAIN_MAX; i++) {
		memset(extend_pass_domains[i], 0, DOMAIN_LEN);
	}
	domain_count = 0;

	fd = fopen(EXTEND_PASS_DOMAIN_FILE, "r");
	if(fd == NULL) {
		fprintf(stderr, "can't open file %s\n", EXTEND_PASS_DOMAIN_FILE);
		return -1;
	}

	while(fgets(line, DOMAIN_LEN, fd) != NULL) {
		strncpy(extend_pass_domains[domain_count], line, strlen(line));
		extend_pass_domains[domain_count][strlen(line)-1] = '\0';
		domain_count++;
