...
#ifdef DNI_IPV6_FEATURE
  /* diff_svr == 1, at least one IPv4 DNS server and one IPv6 DNS server configed diff_svr == 0, DNS servers are all IPv4 or IPv6 */
  int diff_svr;
#endif
...
#define DOMAIN_LEN 1024
#define DOMAIN_MAX 50
extern char extend_pass_domains[DOMAIN_MAX][DOMAIN_LEN];
extern int domain_count;
extern int load_extend_pass_domain();
#endif
