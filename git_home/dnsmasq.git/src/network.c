...
#  endif
  ...
  create_raw_ipv6_listener(&ll6, daemon->port);
  if (l) 
    l->next = l6;
  else 
    l = l6;
  ...
  for (count = 0, serv = daemon->servers; serv; serv = serv->next)
      ...
      if (!(serv->flags & (SERV_LITERAL_ADDRESS | SERV_NO_ADDR | SERV_USE_RESOLV | SERV_NO_REBIND)))
	{
      ...
      if (serv->flags & SERV_FROM_RESOLV)
      {
	      if (NULL == sresolv)
		      sresolv = serv;
	      if (serv->addr.sa.sa_family != sresolv->addr.sa.sa_family)
	      {
		      daemon->diff_svr = 1;
	      }
      }
...
int reload_servers(char *fname)
{
  ...
  while ((line = fgets(daemon->namebuff, MAXDNAME, f)))
	...
	continue;
      ...
      add_update_server(SERV_FROM_RESOLV, &addr, &source_addr, wan_ifname, NULL);
      gotone = 1;
    }
