...
#ifdef DNI_PARENTAL_CTL
  { "    --parental-control[=<file>]", gettext_noop("Enable Parental Control and specify deviceid file(default to /tmp/parentalcontrol.conf)."), NULL },
#endif
#ifdef SUP_STATIC_PPTP
  { "    --static-pptp[=<file>]", gettext_noop("Support static pptp for Russian Version(file default to /tmp/pptp.conf)."), NULL },
#endif
#ifdef BIND_SRVSOCK_TO_WAN
  { "    --wan-interface=name", gettext_noop("interface name of wan, such as \"eth0\" or \"ppp0\"."), NULL },
#endif
  { NULL, NULL, NULL }
}; 
	...
	*up = new;
	...
	break;
      ...
      case LOPT_PARENTAL_CONTROL:
	    ...
	    if (arg) {
	        pc_table_file = safe_string_alloc(arg);
	    }
	    break;
	}
#endif
      ...
      case LOPT_STATIC_PPTP:
        {
            if (arg) {
               daemon->static_pptp_conf = safe_string_alloc(arg);
            } else {
      ...
      case LOPT_WAN_INTERFACE:
        {
	    wan_ifname = safe_string_alloc(arg);
	    break;
	}
#endif
    }
