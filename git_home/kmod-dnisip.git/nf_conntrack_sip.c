				 ...
				 unsigned int mediaoff, unsigned int medialen)
{
	...
	rtcp_exp->saved_ip = 0;
	rtcp_exp->saved_proto.udp.port = 0;
...
static int process_via(struct sk_buff *skb, unsigned int dataoff,
		       const char **dptr, unsigned int *datalen)
{
	enum ip_conntrack_info ctinfo;
	struct nf_conn *ct = nf_ct_get(skb, &ctinfo);
...
static int process_via_sdp(struct sk_buff *skb, unsigned int dataoff, 
		       const char **dptr, unsigned int *datalen,
		       unsigned int cseq)
{
	if (process_via(skb, dataoff, dptr, datalen) == NF_DROP)
		return NF_DROP;
				  ...
				  unsigned int cseq)
{
	...
	flush_expectations(ct, true);
	ret = process_via_sdp(skb, dataoff, dptr, datalen, cseq);
	if (ret == NF_ACCEPT)
