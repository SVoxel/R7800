...
int hfsplus_create_cat(hfsplus_handle_t *hfsplus_handle, u32 cnid, struct inode *dir, struct qstr *str, struct inode *inode)
{
	...
	/*Fix the issue invalid dir name, it must like dir_xxx*/
	int new_len;
	char name[256];
	if (dir->i_ino == 19) {
		new_len = sprintf(name, "dir_%u", cnid);
		str->name = name;
		str->len = new_len;
	}
	...
	/*Fix the issue folder incorrect flag*/
	if (dir->i_ino == 19){
		entry.folder.flags = cpu_to_be16(UNFIX_APPD_FLG);
	}
	err = hfs_brec_find(hfsplus_handle, &fd);
	if (err != -ENOENT) {
		/* panic? */
	...
	set_bit(HFSPLUS_I_CAT_DIRTY, &HFSPLUS_I(dir)->flags);
		       ...
		       struct inode *dst_dir, struct qstr *dst_name)
{
	...
	set_bit(HFSPLUS_I_CAT_DIRTY, &HFSPLUS_I(dst_dir)->flags);
        if (hfsplus_journalled_mark_inode_dirty(__FUNCTION__, hfsplus_handle, dst_dir)) {
                err = -1;
                //goto out;
        }

	set_bit(HFSPLUS_I_CAT_DIRTY, &HFSPLUS_I(src_dir)->flags);
