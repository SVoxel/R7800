/*
 * linux/fs/hfsplus_journal/checkpoint.c
 *
 ...
 * Called under hfsplus_jbd_lock_bh_state(hfsplus_jh2bh(jh)), and drops it
 */
static int __process_buffer(hfsplus_jbd_t *journal, struct hfsplus_jbd_head *jh,
...
int hfsplus_jbd_log_do_checkpoint(hfsplus_jbd_t *journal)
{
	...
	jbd_debug(1, "cleanup_hfsplus_jbd_tail returned %d\n", result);
	if (result <= 0)
		return result;
...
int cleanup_hfsplus_jbd_tail(hfsplus_jbd_t *journal)
{
	 ...
	 * --- perhaps by hfsplus_jbd_log_do_checkpoint() --- are flushed out before we
	 * drop the transactions from the journal. It's unlikely this will be
	 * necessary, especially with an appropriately sized journal, but we
	 * need this to guarantee correctness.  Fortunately
	 * cleanup_hfsplus_jbd_tail() doesn't get called all that often.
	 */
 ...
 * This function is called with hfsplus_jbd_lock_bh_state(hfsplus_jh2bh(jh))
 */
...
int __hfsplus_jbd_remove_checkpoint(struct hfsplus_jbd_head *jh)
{
	...
	HFSPLUS_JBUFFER_TRACE(jh, "removing from transaction");
	__buffer_unlink(jh);
			       ...
			       hfsplus_transaction_t *transaction)
{
	...
	hfsplus_jbd_grab_journal_head(hfsplus_jh2bh(jh));
