...
static void __hfsplus_ext_write_extent(hfsplus_handle_t *hfsplus_handle, struct inode *inode,
		struct hfs_find_data *fd)
{
	...
	if (hip->extent_state & HFSPLUS_EXT_NEW) {
		if (res != -ENOENT)
			return;
		hfs_brec_insert(hfsplus_handle,fd, hip->cached_extents,
				sizeof(hfsplus_extent_rec));
		hip->extent_state &= ~(HFSPLUS_EXT_DIRTY | HFSPLUS_EXT_NEW);
	} else {
		if (res)
			return;
		hfs_bnode_write(hfsplus_handle,fd->bnode, hip->cached_extents,
...
static int hfsplus_ext_write_extent_locked(hfsplus_handle_t *hfsplus_handle, struct inode *inode)
{
	int res;
...
int hfsplus_ext_write_extent(hfsplus_handle_t *hfsplus_handle, struct inode *inode)
{
	int res;

	mutex_lock(&HFSPLUS_I(inode)->extents_lock);
	res = hfsplus_ext_write_extent_locked(hfsplus_handle,inode);
...
static inline int __hfsplus_ext_cache_extent(hfsplus_handle_t *hfsplus_handle, struct hfs_find_data *fd,
		struct inode *inode, u32 block)
{
	...
	res = __hfsplus_ext_read_extent(hfsplus_handle, fd, hip->cached_extents, inode->i_ino,
...
int hfsplus_free_fork(hfsplus_handle_t *hfsplus_handle, struct super_block *sb, u32 cnid,
...
int hfsplus_file_extend(hfsplus_handle_t *hfsplus_handle, struct inode *inode)
{
	...
	if (hip->alloc_blocks == hip->first_blocks)
		...
		res = hfsplus_ext_read_extent(hfsplus_handle, inode, hip->alloc_blocks);
		if (res)
			goto out;
	...
	start = hfsplus_block_allocate(hfsplus_handle, sb, sbi->total_blocks, goal, &len);
...
void hfsplus_file_truncate(struct inode *inode)
{
	...
	while (1) {
		if (alloc_cnt == hip->first_blocks) {
			hfsplus_free_extents(&hfsplus_handle, sb, hip->first_extents,
		...
		hfsplus_free_extents(&hfsplus_handle, sb, hip->cached_extents,
