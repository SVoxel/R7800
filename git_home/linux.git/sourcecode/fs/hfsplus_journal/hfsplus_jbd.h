...
/*
 * Define HFSPLUS_JBD_PARANOID_IOFAIL to cause a kernel BUG() if ext3 finds
 * certain classes of error which can occur due to failed IOs.  Under
 * normal use we want ext3 to continue after such errors, because
 * hardware _can_ fail, but for debugging purposes when running tests on
 * known-good hardware we may want to trap these errors.
 */
#undef HFSPLUS_JBD_PARANOID_IOFAIL
...
/**
 * struct hfsplus_jbd_handle_s - this is the concrete type associated with hfsplus_jbd_handle_t.
...
struct hfsplus_handle {
	...
	u16 hcnt; /* Count number of call of hfsplus_jbd_dirty_metadata() */
	u16 maxblock; 
	u32 journaled;
};
...
struct hfsplus_transaction_s
{
	 ...
	 * [j_list_lock] guards transition of a transaction into HFSPLUS_T_FINISHED
	 * state and subsequent call of __hfsplus_jbd_drop_transaction()
...
/**
 * struct hfsplus_jbd_s - this is the concrete type associated with hfsplus_jbd_t.
 ...
 * @j_hfsplus_transaction_sequence: Sequence number of the next transaction to grant
...
extern void	hfsplus_jbd_clear_buffer_revoked_flags(hfsplus_jbd_t *journal);
