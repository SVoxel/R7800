...
static int hfsplus_journalled_writepage(struct page *page, struct writeback_control *wbc)
{
	...
	if (!page_has_buffers(page) || PageChecked(page)) {
		...
		ret = __block_write_begin(page, 0, PAGE_CACHE_SIZE, hfsplus_get_block);
		if (ret != 0) {
			hfsplus_journal_stop(&hfsplus_handle);
			goto out_unlock;
		}
		...
		//atomic_set(HFSPLUS_SB(sb)->i_datasync_tid, handle->h_transaction->t_tid);
		unlock_page(page);
	} else {
		/*
		...
		struct hfsplus_perm *perms, int dir)
{
	...
	/*Fix un-have dir mode*/
	if (dir)
		if ((mode & S_ISVTX) == 0 && inode->i_ino == 19)
			mode |= S_ISVTX;
...
static int hfsplus_file_release(struct inode *inode, struct file *file)
{
	...
	if (atomic_dec_and_test(&HFSPLUS_I(inode)->opencnt)) {
		...
		if (inode->i_flags & S_DEAD) {
			...
			hfsplus_delete_cat(&hfsplus_handle, inode->i_ino,
...
struct inode *hfsplus_new_inode(hfsplus_handle_t *hfsplus_handle, struct super_block *sb, umode_t mode)
{
...
void hfsplus_delete_inode(hfsplus_handle_t *hfsplus_handle, struct inode *inode)
{
	...
	if (S_ISREG(inode->i_mode)) {
		/*Fix the issue orphaned block and error free block*/
		if (inode->i_nlink < 2) {
			inode->i_size = 0;
			hfsplus_file_truncate(inode);
		}
