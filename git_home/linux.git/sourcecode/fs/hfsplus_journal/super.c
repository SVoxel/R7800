...
static int hfsplus_system_write_inode(hfsplus_handle_t *hfsplus_handle, struct inode *inode)
{
		...
		struct writeback_control *wbc)
{
	...
	err = hfsplus_ext_write_extent(&hfsplus_handle, inode);
	if (err)
		return err;
	...
	if (inode->i_ino >= HFSPLUS_FIRSTUSER_CNID ||
		...
		return hfsplus_system_write_inode(&hfsplus_handle, inode);
}
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		...
		if (!sbi->hidden_dir) {
			if (hfsplus_journal_start(__FUNCTION__, sb, &hfsplus_handle))
				goto out;
			mutex_lock(&sbi->vh_mutex);
			sbi->hidden_dir = hfsplus_new_inode(&hfsplus_handle, sb, S_IFDIR);
			if (!sbi->hidden_dir) {
				mutex_unlock(&sbi->vh_mutex);
				err = -ENOMEM;
				goto out_put_root;
			}
			err = hfsplus_create_cat(&hfsplus_handle, sbi->hidden_dir->i_ino, root,
						 &str, sbi->hidden_dir);
			mutex_unlock(&sbi->vh_mutex);
			if (err)
				goto out_put_hidden_dir;

			set_bit(HFSPLUS_I_CAT_DIRTY, &HFSPLUS_I(sbi->hidden_dir)->flags);
			hfsplus_journalled_mark_inode_dirty(__FUNCTION__, &hfsplus_handle, sbi->hidden_dir);
			hfsplus_journal_stop(&hfsplus_handle);
		}
	}
