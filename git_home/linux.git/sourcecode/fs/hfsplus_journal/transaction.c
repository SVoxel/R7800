/*
 * linux/fs/hfsplus_journal/transaction.c
 *
			...
			int force_copy)
{
	...
	hfsplus_jbd_debug(5, "hfsplus_jbd_head %p, force_copy %d\n", jh, force_copy);
...
repeat:
	...
	if (need_copy) {
		...
		offset = offset_in_page(hfsplus_jh2bh(jh)->b_data);
...
int hfsplus_jbd_get_create_access(hfsplus_jbd_handle_t *handle, struct buffer_head *bh)
{
	...
	if (jh->b_transaction == NULL) {
		/*
		 * Previous hfsplus_jbd_forget() could have left the buffer
...
static int hfsplus_jbd_unmap_buffer(hfsplus_jbd_t *journal, struct buffer_head *bh)
{
	 ...
	 * the buffer to HFSPLUS_BJ_Forget list of the newest transaction.
	 */
	transaction = jh->b_transaction;
	if (transaction == NULL) {
		...
		if (journal->j_running_transaction && buffer_hfsplus_jbddirty(bh))
			...
			hfsplus_transaction_t *transaction, int jlist)
{
	...
	if (jlist == HFSPLUS_BJ_Metadata || jlist == HFSPLUS_BJ_Reserved ||
	    jlist == HFSPLUS_BJ_Shadow || jlist == HFSPLUS_BJ_Forget) {
		/*
		 * For metadata buffers, we track dirty bit in buffer_hfsplus_jbddirty
...
void __hfsplus_jbd_refile_buffer(struct hfsplus_jbd_head *jh)
{
	 ...
	 * our jh reference and thus __hfsplus_jbd_file_buffer() must not take a
	 * new one.
	 */
	jh->b_transaction = jh->b_next_transaction;
	jh->b_next_transaction = NULL;
	if (buffer_hfsplus_jbd_freed(bh))
		jlist = HFSPLUS_BJ_Forget;
	else if (jh->b_modified)
	//if (jh->b_modified)
		jlist = HFSPLUS_BJ_Metadata;
	else
		jlist = HFSPLUS_BJ_Reserved;
	__hfsplus_jbd_file_buffer(jh, jh->b_transaction, jlist);
...
/*
 * __hfsplus_jbd_refile_buffer() with necessary locking added. We take our bh
