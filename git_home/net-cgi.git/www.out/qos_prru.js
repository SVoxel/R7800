...
function setGray()
{
	...
	if(cf.qosEnable.checked == true)
	{
		...
		if(cf.qos_mode[0].checked == true)
		{
			...
			cf.Deleteone.className = "delete_bt";
			cf.DeleteAll.disabled = false;
			cf.DeleteAll.className = "delete_bt";
		}
		else
		{
	...
	else
	{
		...
		if(cf.qos_mode[0].checked == true)
		{
			...
			cf.Deleteone.className = "delete_gray_bt";
			cf.DeleteAll.disabled = true;
			cf.DeleteAll.className = "delete_gray_bt";
		}
		else
		{
...
function add_rules()
{
	...
	if(cf.qos_type[0].checked == true)
	{
		...
		if((parent.is_pr_version == 1 && index == 40) || (parent.is_ru_version == 1 && index == 33) || cf.qos_apps.selectedIndex == 31)
		{
	...
	cf.action="/apply.cgi?/QOS_prru_main.htm timestamp=" +ts;
	return true;
}
...
function check_qos_add_info()
{
	...
	if(check_name_space(cf.ser_poli_name.value)==false)
		return false;
	if((parent.is_pr_version == 1 && index == 40) || (parent.is_ru_version == 1 && index == 33))
	{
