...
struct RemoteActivation {
	struct {
		...
		struct GUID Clsid;
		const char *pwszObjectName;/* [charset(UTF16),unique] */
		struct MInterfacePointer *pObjectStorage;/* [unique] */
		enum imp_levels ClientImpLevel;
		uint32_t Mode;
		uint32_t Interfaces;/* [range(1,32768)] */
		struct GUID *pIIDs;/* [size_is(Interfaces),unique] */
	...
	struct {
		...
		HRESULT *results;/* [size_is(Interfaces)] */
		WERROR result;
	} out;
