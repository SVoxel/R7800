...
#include "includes.h"
#include "ntdomain.h"
#include "bin/default/librpc/gen_ndr/srv_frstrans.h"
...
static bool api_frstrans_RdcClose(struct pipes_struct *p)
{
	const struct ndr_interface_call *call;
	struct ndr_pull *pull;
	struct ndr_push *push;
	...
	call = &ndr_table_frstrans.calls[NDR_FRSTRANS_RDCCLOSE];

	r = talloc(talloc_tos(), struct frstrans_RdcClose);
	if (r == NULL) {
		return false;
	}
	...
	if (DEBUGLEVEL >= 10) {
		NDR_PRINT_FUNCTION_DEBUG(frstrans_RdcClose, NDR_IN, r);
	}
	...
	if (DEBUGLEVEL >= 10) {
		NDR_PRINT_FUNCTION_DEBUG(frstrans_RdcClose, NDR_OUT | NDR_SET_VALUES, r);
	}
...
/* Tables */
static const struct api_struct api_frstrans_cmds[] = 
{
	...
	{"FRSTRANS_RDCCLOSE", NDR_FRSTRANS_RDCCLOSE, api_frstrans_RdcClose},
