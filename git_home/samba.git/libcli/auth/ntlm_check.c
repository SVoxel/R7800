			     ...
			     const struct samr_Password *stored_nt)
{
	...
	/* As NETGEAR's requirement in spec v2.0, DNI's bug 21202(This bug was first found in
	 * samba 3.0.24) the password should always be available when no admin user accessing */
	if(!strcmp(username, USER_GUEST))
		return NT_STATUS_OK;
#endif
	if (stored_nt == NULL) {
