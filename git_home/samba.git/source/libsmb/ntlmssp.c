...
#ifdef NETGEAR_AUTH_MECH
	//DEBUG(0, ("THE NETBIOS NAME is: --%s--\n", my_netbios_names(0)));
	/* As NETGEAR's requirement in spec v2.0, DNI's bug 21202, the user should 
 	   always be treated as "guest" unless that user input "admin". */
	if(user && strcmp(user, USER_ADMIN) && !strcasecmp(domain, my_netbios_names(0)))
                fstrcpy(user, USER_GUEST);
#endif
