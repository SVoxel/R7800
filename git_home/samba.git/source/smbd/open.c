			...
			BOOL file_existed)
{
	...
	if(S_ISDIR(info.st_mode) || !file_existed)	/*fix can not open url'\\readyshare' in win7 */
		return SMB_VFS_OPEN(conn,fname,flags,mode);
	...
	if(fd < 0){
		 ...
		 */
		return SMB_VFS_OPEN(conn,fname,oldflags,mode);
	}

	DEBUG(10,("fd_open: name %s, flags = 0%o, oldflags = 0%o, mode = 0%o, fd = %d. %s\n", fname,
	              flags, oldflags, (int)mode, fd, (fd == -1) ? strerror(errno) : "" ));
  	...
  	if(lock.l_type == F_UNLCK){
		...
		if(flagstmp == O_CREAT){
			close(fd);
			fd = SMB_VFS_OPEN(conn,fname,oldflags,mode);
		}
 		lock.l_type = F_WRLCK;
		lock.l_start = 0;
 		lock.l_whence = SEEK_SET;
			...
			uint32 open_access_mask) /* what we're actually using in the open. */
{
	...
	if ((open_access_mask & (FILE_READ_DATA|FILE_WRITE_DATA|FILE_APPEND_DATA|FILE_EXECUTE)) ||
		...
		fsp->fh->fd = fd_open(conn, fname, local_flags, unx_mode, file_existed);
		if (fsp->fh->fd == -1)  {
