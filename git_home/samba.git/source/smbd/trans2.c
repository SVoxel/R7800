...
/*	if ((dosmode != 0) && (dosmode != dos_mode(conn, fname, &sbuf))) {
	...
	if (size != get_file_size(sbuf)) {
		...
		if (fd == -1) {
			...
			/* DNI bug 14263 when write large file about 750M to FAT disk,always get error message
			  * vfs_set_filelen need more time to flush cache with FAT fs,windows side may timeout,
			  * so remove the function refer WGT634U modified*/
