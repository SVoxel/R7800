		 ...
		 bool file_existed)
{
	...
	/*
	 * bug25349
	 * if open config files and creat new file skip lock file to avoid other acess write it
	 */
	struct stat info;
	stat(smb_fname_str_dbg(smb_fname), &info);
	if(S_ISDIR(info.st_mode) || !file_existed){     /*fix can not open url'\\readyshare' in win7 */
	...
	if (fsp->fh->fd == -1) {
		...
		DEBUG(0,("!!!FAIL to open file %s\n", smb_fname_str_dbg(smb_fname)));
		/*
		 * if usbstorage set to write-protect mode. we could not lock file
		 * with write permission, and not necessary lock it.
		 */
		fsp->fh->fd = SMB_VFS_OPEN(conn, smb_fname, fsp, oldflags, mode);
		if (fsp->fh->fd == -1) {
			status = map_nt_error_from_unix(errno);
			if (errno == EMFILE) {
				static time_t last_warned = 0L;
	...
	DEBUG(10,("fd_open: name %s, flags = 0%o, oldflags = 0%o, mode = 0%o, fd = %d. %s\n",
		  smb_fname_str_dbg(smb_fname), flags, oldflags, (int)mode, fsp->fh->fd,
	...
	lock.l_len = 0;
	if(fcntl(fsp->fh->fd, F_GETLK, &lock) < 0)
		DEBUG(10,("!!!FAIL to get lock info of %s\n", smb_fname_str_dbg(smb_fname)));

	if(lock.l_type == F_UNLCK){
		/*
		 * when file already existed in sharefolder and didn't be accessed by
		 * other access.use the oldflags to open it.
		 */
		if(flagstmp == O_CREAT){
			close(fsp->fh->fd);
			fsp->fh->fd = SMB_VFS_OPEN(conn, smb_fname, fsp, oldflags, mode);
		...
		lock.l_whence = SEEK_SET;
		lock.l_len = 0;
		if(fcntl(fsp->fh->fd, F_SETLK, &lock) < 0){
			DEBUG(10,("!!!FAIL to lock file %s, fd = %d\n", smb_fname_str_dbg(smb_fname), fsp->fh->fd));
			return status;
		}else{
			DEBUG(10,("SUCCESS to lock file %s\n", smb_fname_str_dbg(smb_fname)));
			return status;
		}
	/* if just read file , open it*/
	}else if((oldflags & O_ACCMODE) == O_RDONLY){
		close(fsp->fh->fd);
		fsp->fh->fd = SMB_VFS_OPEN(conn, smb_fname, fsp, oldflags, mode);
		if (fsp->fh->fd == -1) {
			status = map_nt_error_from_unix(errno);
		...
		DEBUG(10,(" !!! file %s was locked before\n", smb_fname_str_dbg(smb_fname)));
			...
			bool *file_created)
{
	...
	*file_created = false;
	...
	if (!(flags & O_CREAT)) {
		 ...
		 */
		return fd_open(conn, fsp, flags, mode, file_existed);
	}
	...
	if (flags & O_EXCL) {
		 ...
		 */
		status = fd_open(conn, fsp, flags, mode, file_existed);
	...
	status = fd_open(conn, fsp, curr_flags, mode, file_existed);
	if (NT_STATUS_IS_OK(status)) {
		if (!file_existed) {
			*file_created = true;
		}
			       ...
			       files_struct **result)
{
	...
	*/
	...
	if (access_mask & (FILE_LIST_DIRECTORY|
			   ...
			   FILE_DELETE_CHILD)) {
#ifdef O_DIRECTORY
		status = fd_open(conn, fsp, O_RDONLY|O_DIRECTORY, 0, dir_existed);
#else
		/* POSIX allows us to open a directory with O_RDONLY. */
		status = fd_open(conn, fsp, O_RDONLY, 0, dir_existed);
#endif
		if (!NT_STATUS_IS_OK(status)) {
