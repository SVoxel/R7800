 ...
 *		chaddr - in/out, MAC address of yiaddr
 * retn: 	1 addr free
 *		0 addr used
 *		-1 error 
 */  
...
int arpping(u_int32_t yiaddr, u_int32_t ip, unsigned char *mac, char *interface, u_int8_t *chaddr)
{
	int	times = 2;
	int 	optval = 1;
	int	s;			/* socket */
	...
	while (times > 0) {
		...
		FD_SET(s, &fdset);
		tm.tv_sec  = 0;
		tm.tv_usec = 100000; /* 100ms * 2 = 200ms SHOULD be OK! */
