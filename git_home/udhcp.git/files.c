...
#ifdef DHCPD_STATIC_LEASE
static void add_staticlease(struct static_lease *lease, struct static_lease **head)
{
...
/* static_lease 192.168.1.88 00:14:22:8E:42:0D DNI-TEST */
static int read_staticlease(char *line, void *arg)
{
	char *ip, *mac;
	unsigned int ipaddr;
	struct ether_addr *ethaddr;
	struct static_lease *newlease;
	...
	if (ipaddr == INADDR_NONE)
		goto ret;
	ethaddr = ether_aton(mac);
	if (ethaddr == NULL)
		goto ret;

	newlease = malloc(sizeof(struct static_lease));
	if (newlease == NULL)
		goto ret;

	memcpy(newlease->mac, ethaddr->ether_addr_octet, 6);
	newlease->ip = ipaddr;
	newlease->next = NULL;

	add_staticlease(newlease, arg);

	LOG(LOG_INFO, "Adding static leases: %s %s", ip, mac);
...
int read_config(char *file)
{
	FILE *in;
#ifdef DHCPD_STATIC_LEASE
	char buffer[512], orig[512], *token, *line;
#else
	...
	while (fgets(buffer, 80, in)) {
		...
		if (strchr(buffer, '\n')) *(strchr(buffer, '\n')) = '\0';
#ifdef DHCPD_STATIC_LEASE
		strncpy(orig, buffer, sizeof(orig));
#else
