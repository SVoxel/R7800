...
#define __QUOTE_MAC__(x) __QUOTE__(x)
#define common_log(...) libmtd_log(__VA_ARGS__)
#define dni_libmtd_log(fmt, args...) common_log(" [" __FILE__ "][%s](" __QUOTE_MAC__(__LINE__) "): "fmt, __func__, ##args)
 ...
 */
extern void libmtd_log(const char *fmt, ...);
/**
 * dni_mtd_write - write data to MTD device.
 * @mtd_device: MTD device description object
 * @img: data file that will be write
 * @imgoffset: offset of data file that will be write
 * @buf: data that will be write
 * @len: length of data that will be write
 * @mtdoffset: offset of mtd block that will be write
 *
 * This function writes data to the MTD device defined by @mtd. @buf @len
 */
extern int dni_mtd_write(const char *mtd_device, char *img, long long imgoffset, char *buf, int len, long long mtdoffset);

/**
 * dni_mtd_read - read data from MTD device.
 * @mtd_device: MTD device description object
 * @buf: data that will be write
 * @len: length of data that will be write
 *
 * This function read data from the MTD device defined by @mtd. @buf @len
 */
extern int dni_mtd_read(const char *mtd_device, char *buf, int len);
#ifdef __cplusplus
}
#endif
