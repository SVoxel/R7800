...
bin=${0##*/}
opvncfg_dir="${ovpn_client_cfg_dir}/${purevpn}"
...
connect_server()
{
	...
	while ! ls ${ovpn_client_cfg_dir}/${purevpn}/${1}-${2}*-${proto}.ovpn >/dev/null 2>&1
	do
		sleep 3
	done
	...
	for cfg_file in $prior_cfg_file \
			${ovpn_client_cfg_dir}/${purevpn}/${1}-${2}*-${proto}.ovpn
	do
