...
LoadInput (void)
{
  ...
  fp = fopen(getenv("CGI_POST_TMPFILE"), "r");
  ...
  if (strcmp (method, "GET") == 0) {
    ...
    if(fp){
    	if ((ts = fread (result, sizeof (char), length, fp)) < length) {
      		fprintf (stderr, "proccgi -- error reading post data, %d bytes read, %d expedted\n",
	      		ts, length);
    	}
	fclose(fp);
	//unlink(getenv("CGI_POST_TMPFILE"));
    }
    else{
    ...
    {
	    FILE *aabbcc;
	    aabbcc = fopen("/tmp/ronger_debug", "w");
	    fprintf(aabbcc, getenv("CGI_POST_TMPFILE"));
	    fprintf(aabbcc, result);
	    fclose(aabbcc);
    }
#endif
  }
