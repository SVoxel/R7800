...
define Host/Configure
	...
	$(SED) 's,^KERNEL_HEADERS=.*,KERNEL_HEADERS=\"$(BUILD_DIR_TOOLCHAIN)/linux-dev/include\",g' \
		...
		-e 's,^.*UCLIBC_HAS_SSP[= ].*,UCLIBC_HAS_SSP=$(if $(CONFIG_SSP_SUPPORT),y,n),g' \
		-e 's,^.*UCLIBC_BUILD_NOW.*,UCLIBC_BUILD_NOW=$(if $(CONFIG_ENABLE_RELRO),y,n),g' \
		-e 's,^.*UCLIBC_BUILD_PIE.*,UCLIBC_BUILD_PIE=$(if $(CONFIG_ENABLE_RELRO),y,n),g' \
